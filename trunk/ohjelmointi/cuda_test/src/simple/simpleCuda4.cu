...
int main(void)
{
	...
	cudaStat += cudaMalloc((void **) &pCudaArgument, N*sizeof(pCudaResult[0]));
	cudaStat += cudaMalloc((void **) &pCudaResult, N*sizeof(pCudaResult[0]));

	// initialize argument
	for (i = 0; i < N; i++) pHostArgument[i] = float(i);

	// copy argument from host to cuda
	cudaStat += cudaMemcpy(	pCudaArgument, 				// destination
	...
	cudaStat +=	cudaMemcpy(	pHostResult, 				// destination
							pCudaResult, 				// source
							N*sizeof(pCudaResult[0]),	// amount to copy
