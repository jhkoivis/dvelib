...
class MyService(Thread, Verbose):
    ...
    def run(self):
        ...
        self.server = MySocketServer(self.port, verbose = self.verbose)
...
class MyChannel(asyncore.dispatcher, Verbose):
    ...
    def __init__(self, channel, verbose = 10, text = None):
        Verbose.__init__(self, verbose = verbose)
    ...
    def handle_write(self):
        ...
        if not self.written:
            ...
            self.written = 1
        if self.written and self.recieved:
            self.close()
    ...
    def handle_read(self):
        ...
        if not self.recieved:
            print s
            md = mouseDragger.MouseDragger()
            md.leftDrag(md.parseString(s))
            self.recieved = 1
        if self.written:
            self.close()
       
    def handle_close(self):
        self.printVerbose(2, 'Handle Close')
        self.close()
